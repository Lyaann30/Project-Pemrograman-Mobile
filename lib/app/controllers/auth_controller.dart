...
Future<bool> loginUser(String email, String password) async {
  ...
  //Method untuk menambah data pembelian dari Alamat Tagihan
  ...
  required String address,
    ...
    try {
      ...
      QuerySnapshot cartSnapshot = await FirebaseFirestore.instance
          .collection('cart')
          .where('uid', isEqualTo: user.uid)
          .where(FieldPath.documentId, whereIn: selectedItemIds)
          .get();
      ...
      if (selectedItemIds.length > 1) {
        // Multiple items
        productData = cartSnapshot.docs.map((doc) {
        ...
        // Single item
        final item = cartSnapshot.docs.first.data() as Map<String, dynamic>;
        productData = {
          'imageUrl': item['imageUrl'],
          'nameProduct': item['name'],
          'price': item['price'],
      ...
      // Calculate total
      double total = cartSnapshot.docs.fold(0, (sum, doc) {
        final item = doc.data() as Map<String, dynamic>;
      ...
      // Add checkout data
      await _firestore.collection('checkout').add({
        'product': productData,
        'total': total,
        'nameUser': name,
        'phone': phone,
